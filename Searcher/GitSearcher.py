...
from GoogleSearcher import GoogleSearcher, GoogleAjaxSearch
import utils
...
class GoogleStackoverflowSearcher:
	...
	def search(self, query, limit=1):
		...
		#g = GoogleSearcher()
		# q_ids = g.so_ranking(query)
		g = GoogleSearcher()
		q_ids = g.search(query)
		i = 0
		for i, q_id in enumerate(q_ids):
			query = TermQuery(Term("id", q_id));
			topdocs = self.searcher.search(query, 5).scoreDocs

			for hit in topdocs:
				doc = self.searcher.doc(hit.doc)
				docs.append( SOResultItem(doc, len(q_ids) - i, doc.get("title"), doc.get("id"), doc.get("description")) ) #len(q_ids) - i , doc.get("view_count") MAX VIEWCOUNT: 1.914.994
		...
		if query:
			...
			try:
				...
				for i, hit in enumerate(hits):
					...
					file_path = "/extdsk/FaCoY/Git_data/G" + doc.get("file")[24:]
					print(file_path)
					content = None
					try:
