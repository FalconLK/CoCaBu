...
class StackoverflowParser:
	...
	def _stackoverflow_links(self, n=3):
		...
		print "SOParser", url
		d = get_json( url)
		print "SO LINKS", len(d["items"])
		links = [ {"title": item["title"], "link": item["link"]} for item in d["items"][:n]]
	...
	def get_html(self, n = 10):
		...
		elements = []
		so_q = self._stackoverflow_links(n)
		for so_question in so_q:
		...
		return "%s Results. Your search returned no matches." % (len(so_q))
